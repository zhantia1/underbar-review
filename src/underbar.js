(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if(arguments.length < 3) {
      return _.filter(copy, function(value) {
        return results.includes(value) ? null : results.push(value);
      })      
    }
    
    _.each(copy, function(value) {
      if(!(unique.includes(iterator(value)))) {
        unique.push(iterator(value));
        results.push(value);
      }
    });
  ...
  _.shuffle = function(array) {
    ...
    for(var i = 0; i < array.length; i++) {
      var randomIndex = Math.floor(Math.random() * Math.floor(copy.length));
      temp = copy[i];
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    for(var i = 0; i < collection.length; i++) {
      if(typeof functionOrKey === "string") {
        results.push("test"[functionOrKey].apply(collection[i]));
